...
class UserController extends Controller
{
    ...
    public function index(Request $request)
    {
        $users = User::where(function($query) use ($request) {
            ...
            $query->whereIn('roles', ['receptionist','accounting','admin']);
        })
    ...
    public function store(Request $request)
    {
        ...
        if($store)
            return response()->json(['status' => 'success', 'message' => 'Success! data created successfully', 'next' => Route('backend.users.index')]);
        else
    ...
    public function update(Request $request, string $id)
    {
        ...
        if($update)
            return response()->json(['status' => 'success', 'message' => 'Success! data updated successfully', 'next' => Route('backend.users.index')]);
        else
            return response()->json(['status' => 'error', 'message' => 'Oops! something went wrong, data not saved'], 422);
    }
