...
@section('content')
  <div class="container-xxl flex-grow-1 container-p-y">
    ...
    <h5 class="mt-4 mb-4">Detail Payment Request</h5>
    <div class="card mb-4">
      <div class="card-body">
        <div class="row">
          <div class="col-md-6 mb-3">
            <label class="form-label">Select Trip</label>
            <select name="select_trip" class="form-select" base_error="Err_select_trip" disabled>
          ...
          <div class="col-md-12 mb-3">
            <label class="form-label">Total Rates Amount</label>
          ...
          <div class="status-cancel-additional-form" style="{{ $paymentRequest->status == 'cancel' ? 'display: block;':'display: none;' }}">
            <div class="col-md-12 mb-3">
              ...
              <textarea name="reason" class="form-control" cols="30" rows="6" base_error="Err_reason" readonly>{!! isset($decReason->cancel) ? $decReason->cancel : '' !!}</textarea>
        ...
        <div class="status-done-additional-form" style="{{ $paymentRequest->status == 'done' ? 'display: block;':'display: none;' }}">
          ...
          <div class="col-md-12 mb-3">
            ...
            <input class="form-control kdef-rate-format" type="text" name="total_payment_rates" base_error="Err_total_payment_rates" value="{{ isset($paymentRequest->re_has_invoices[0]) ? number_format($paymentRequest->re_has_invoices[0]->total_payment_rates, 0, ',', '.') : number_format($paymentRequest->total_rates_amount, 0, ',', '.') }}" readonly>
            <div id="Err_total_payment_rates" class="text-danger"></div>
          </div>
          <div class="col-md-12 mb-3">
            <label class="form-label">Remarks</label>
            <textarea name="remarks" class="form-control" cols="30" rows="5" base_error="Err_remarks" readonly>{!! isset($paymentRequest->re_has_invoices[0]) ? $paymentRequest->re_has_invoices[0]->remarks : '' !!}</textarea>
